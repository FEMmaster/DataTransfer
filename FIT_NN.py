...
init_methods = {
    ...
    'xavier_uniform': lambda x: nn.init.xavier_uniform_(x.weight, gain=1.0),
...
if __name__ == '__main__':
    ...
    X_train_T1, T1_cells, y_train_T1 = MeshTools.convert_torch(T1_p, T1_c, T1_v, dtype, device)
    X_test_T2,  T2_cells,  y_test_T2 = MeshTools.convert_torch(T2_p, T2_c, T2_v, dtype, device)
    X_test_T3,  T3_cells,  y_test_T3 = MeshTools.convert_torch(T3_p, T3_c, T3_v, dtype, device)
    X_test_T4,  T4_cells,  y_test_T4 = MeshTools.convert_torch(T4_p, T4_c, T4_v, dtype, device)
    X_test_T5,  T5_cells,  y_test_T5 = MeshTools.convert_torch(T5_p, T5_c, T5_v, dtype, device)
    ...
    #%% LBFGS训练
    optimizer_lbfgs = optim.LBFGS(model.parameters(), max_iter=10000, tolerance_change=1e-11, line_search_fn="strong_wolfe")
    loss_log_lbfgs = []
    it = [0]      # 使用列表来存储迭代计数
    ...
    # 连续编号的横轴
    n_adam  = len(loss_log_adam)
    n_lbfgs = len(loss_log_lbfgs)
    x_adam  = np.arange(n_adam)
    x_lbfgs = n_adam + np.arange(n_lbfgs)

    # 分段着色便于辨识
    if n_adam:
        ax.plot(x_adam, loss_log_adam, lw=1.8, label='Adam')
    if n_lbfgs:
        ax.plot(x_lbfgs, loss_log_lbfgs, lw=1.8, label='L-BFGS')

    # 对数纵轴
    ax.set_yscale('log')
    
    # 标题与坐标轴标题（按你的字号风格）
    ax.set_title('Training Loss (Adam → L-BFGS)', fontsize=16,
                fontweight='bold', family='serif')
    ax.set_xlabel('Step', fontsize=14, fontweight='bold', family='serif')
    ax.set_ylabel('Loss', fontsize=14, fontweight='bold', family='serif')

    # 视觉细节
    ax.tick_params(axis='both', which='major', labelsize=12, direction='in', width=1.0)
