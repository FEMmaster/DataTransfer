...
def generate_2d_meshes(x_domain, y_domain, N):
    ...
    参数:
    x_domain: x方向区间 [x_min, x_max]
    y_domain: y方向区间 [y_min, y_max]
    N: 每条边的节点数量
    ...
    return {'nodes': points_A, 'triangles': tri_A.simplices}, {'nodes': points_B, 'triangles': tri_B.simplices}
...
def interpolate_tri(x_new, x_nodes, y_nodes, tri=None):
    ...
    z = lin(x_new[:, 0], x_new[:, 1])         # MaskedArray
    z = np.asarray(z)                          # 转成 ndarray；凸包外为 nan
    return z

def interpolate_rbf(x_new, x_nodes, y_nodes, L=1000, gamma=50.0):
    """RBF径向基函数插值方法"""
    n, d = x_nodes.shape  # n个样本, d维特征
    L = min(L, n)
    
    # 1) 从训练点随机选择 L 个中心
    idx = np.random.choice(n, size=L, replace=False)
    C = x_nodes[idx]  # 形状 [L, d]
    
    # 2) 构造高斯 RBF 设计矩阵
    # 计算欧几里得距离的平方: ||x_i - c_j||^2
    # 使用广播机制计算所有点对之间的距离
    
    # 对于训练数据: 计算 x_nodes 和 C 之间的距离矩阵
    # x_nodes: [n, d], C: [L, d] -> 差: [n, L, d] -> 平方和: [n, L]
    dist_sq_H = np.sum((x_nodes[:, np.newaxis, :] - C[np.newaxis, :, :])**2, axis=2)  # 修正：axis=2
    H = np.exp(-gamma * dist_sq_H)  # [n, L]
    
    # 对于预测数据: 计算 x_new 和 C 之间的距离矩阵
    dist_sq_K = np.sum((x_new[:, np.newaxis, :] - C[np.newaxis, :, :])**2, axis=2)  # 修正：axis=2
...
def plot_true_solution(func, save_dir, title_offset=0.9):
    ...
    surf_true = ax_true.plot_surface(
        Xg, Yg, Ftrue, cmap="viridis", alpha=0.9, antialiased=True, linewidth=0
    )
    ...
    # cbar.set_label(r"$f(x, y)$", fontsize=cbar_fontsize, fontweight='bold', family='serif')
    ...
    # 坐标轴标签（含 LaTeX 数学字体）
    ax_true.set_xlabel(r"$x$", fontsize=label_fontsize, fontweight='bold', family='serif', labelpad=15)
    ax_true.set_ylabel(r"$y$", fontsize=label_fontsize, fontweight='bold', family='serif', labelpad=15)
    ax_true.set_zlabel(r"$f(x, y)$", fontsize=label_fontsize, fontweight='bold', family='serif', labelpad=15)

    # 刻度字体
    ax_true.tick_params(axis='both', which='major',
                    direction='in', width=1.2, length=8, labelsize=tick_fontsize, pad=5)
    ...
    plt.savefig(true_solution_path, dpi=300, bbox_inches='tight', facecolor='white')
    plt.close(fig_true)
...
def plot_error_fields(A_nodes, B_nodes, tri_A, tri_B, error_A, error_B, filename, method_name, iteration):
    ...
    print(f"正在绘制二维误差图（{method_name}, iter={iteration+1}）...")
    ...
    title1 = f"{method_name} - Iter {iteration} - B→A"
    title2 = f"{method_name} - Iter {iteration} - A→B"
...
class AnisotropicPlotter:
    ...
    def setup_camera(self, center: np.ndarray, elev_deg: float, azim_deg: float, 
        ...
        # 设置固定的正交投影缩放
        self.plotter.camera.parallel_projection = True
        self.plotter.camera.parallel_scale = 0.8  # 固定投影缩放
    ...
    def add_axis_labels(self, orig_bounds: Tuple[float, float, float, float, float, float],
        ...
        for x_val in x_ticks:
            ...
            label_pos = [x_scaled - 0.14 * L_xy, scaled_bounds[2] - 0.13 * L_xy, scaled_bounds[4]]
            self._add_single_label(label_pos, f"{x_val:.1f}", font_size)
        
        # Y轴标签（右侧前缘）
        for y_val in y_ticks:
            y_scaled = center[1] + (y_val - center[1]) * sy
            label_pos = [scaled_bounds[0] - 0.20 * L_xy, y_scaled - 0.15 * L_xy, scaled_bounds[4]]
            self._add_single_label(label_pos, f"{y_val:.1f}", font_size)
            
        # Z轴标签（左侧后角）
        for z_val in z_ticks:
            z_scaled = center[2] + (z_val - center[2]) * sz
            label_pos = [scaled_bounds[0] - 0.12 * L_xy, scaled_bounds[3], z_scaled + 0.03 * L_xy]
    ...
    def plot_mesh(self, nodes: np.ndarray, cells: np.ndarray, node_values: np.ndarray,
                elev_deg: float = 45, azim_deg: float = 226,
        ...
        actor = self.plotter.show_bounds(
            ...
            n_xlabels=6, n_ylabels=6, n_zlabels=4,
            fmt="%.1f",
            font_family='arial',
        )
...
def run_interpolation_experiment(func, A, B, K, interpolate_func, method_name, save_dir, tri_A, tri_B):
    ...
    plotter = AnisotropicPlotter(
        ...
        fixed_z_scale=0.35,      # Z轴缩放为原来的一半
        fixed_bounds=(0, 1, 0, 1, -1.5, 1.5)   # Z轴刻度固定为-1到1
    )
    ...
    for iteration in range(K):  # 包括第0次迭代
        ...
        error_plot_filename = os.path.join(method_save_dir, f"error_iter{iteration+1:02d}.pdf")
...
if __name__ == "__main__":
    # ===================== 可调参数 =====================
    func = f2_multi_peaks      # 选择其中一个二维测试函数
    N = 100    # A/B 节点数量
    K = 200               # 迭代步（一次 A→B + 一次 B→A 算一轮）- 减少次数用于测试

    mesh_A, mesh_B = generate_2d_meshes([0,1], [0,1], N)
    ...
    save_dir = os.path.join(current_dir, "2d_interpolation_animation_peak")
    os.makedirs(save_dir, exist_ok=True)
