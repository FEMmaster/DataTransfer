...
def interpolate_rbf(x_new, x_nodes, y_nodes, L=80, gamma=200.0):
    """1D RBF径向基函数插值方法"""
    n = len(x_nodes)
    L = min(L, n)
...
def update_inset(phase, A, B, yA, yB, inset):
    ...
    # 更新线宽（根据相位）
    inset["la"].set_linewidth(6.0 if phase == 0 else 4.0)
    inset["lb"].set_linewidth(4.0 if phase == 0 else 6.0)
    
    # 更新透明度
    inset["la"].set_alpha(1.0 if phase == 0 else 0.5)
    inset["lb"].set_alpha(0.5 if phase == 0 else 1.0)
    ...
    if phase == 0:
        inset["sa"].set_sizes([100] * len(A))  # A点大
        inset["sb"].set_sizes([60] * len(B))  # B点小
        inset["sa"].set_alpha(1.0)   # A点不透明
        inset["sb"].set_alpha(0.5)   # B点半透明
        inset["sa"].set_zorder(2)    # 与粗线同层
        inset["sb"].set_zorder(3)    # 与细线同层
    else:
        inset["sa"].set_sizes([60] * len(A))  # A点小
        inset["sb"].set_sizes([100] * len(B))  # B点大
        inset["sa"].set_alpha(0.5)   # A点半透明
...
def run_interpolation_experiment(func, A, B, K, interpolate_func, method_name, save_dir,
    ...
    for iteration in range(K):
        ...
        # 误差图
        # 设置专业字体
        fig_err, ax_err = plt.subplots(figsize=(8, 5))  # 增大图形尺寸
        yA_true, yB_true = func(A), func(B)
        err_A, err_B = np.abs(yA - yA_true), np.abs(yB - yB_true)

        # 绘制误差曲线和散点
        ax_err.plot(A, err_A, color="#E67E22", linewidth=2.0, marker='o', markersize=3, 
                label='A (B→A)', zorder=3, alpha=0.8)
        ax_err.plot(B, err_B, color="#2E86C1", linewidth=2.0, marker='o', markersize=3, 
                label='B (A→B)', zorder=3, alpha=0.8)

        # 坐标轴设置
        ax_err.set_xlim(0, 1)
        ax_err.set_xlabel('$x$', fontsize=14, fontweight='bold', family='serif')
        ax_err.set_ylabel('Absolute Error', fontsize=14, fontweight='bold', family='serif')

        # 标题和图例
        ax_err.set_title(f"{method_name} - Error Field (Iteration {iteration+1})", 
                        fontsize=16, fontweight='bold', pad=20)
        ax_err.legend(loc='best', framealpha=0.9, edgecolor='k', fontsize=20)

        # 网格和刻度设置
        ax_err.grid(True, alpha=0.4, linestyle='--', linewidth=0.8)
        ax_err.tick_params(axis='both', which='major', labelsize=20, 
                        direction='in', width=1.2, length=5)

        # 科学计数法显示（如果误差值很小）
        if err_A.max() < 0.01 or err_B.max() < 0.01:
            ax_err.ticklabel_format(axis='y', style='sci', scilimits=(-2,2))
...
if __name__ == "__main__":
    ...
    save_dir = os.path.join(current_dir, "1d_interpolation_animation_peak")
    ...
    INSET_BOX1 = (0.60, 0.45, 0.35, 0.35)
    LOC1_1, LOC1_2 = 1, 3

    # 第二个放大图参数
    ROI2 = (0.68, 0.82, -0.55, -0.30)
    INSET_BOX2 = (0.15, 0.05, 0.25, 0.25)
    LOC2_1, LOC2_2 = 1, 3
    ...
    errors_A_linear, errors_B_linear = run_interpolation_experiment(
        ...
        ROI_list=[ROI1, ROI2],
        INSET_BOX_list=[INSET_BOX1, INSET_BOX2],
        LOC_list=[(LOC1_1, LOC1_2), (LOC2_1, LOC2_2)]
    )
