...
class VisualizationTools:
    ...
    @staticmethod
    def plot_solution_comparison(points, cells, true_solution, predicted_solution, 
        ...
        参数:
            ...
            cells: 网格单元
            true_solution: 真解
            predicted_solution: 预测解  
            save_path: 保存路径
            vrange: 颜色范围 [vmin, vmax]，可选
        """
        ...
        def _setup_subplot(ax, data, title, vrange=None):
            """设置单个子图的格式"""
            
            # 如果传入了范围，就使用指定的vmin和vmax
            if vrange is not None:
                vmin, vmax = vrange
                t = ax.tripcolor(triangulation, data, shading='gouraud', cmap='jet', vmin=vmin, vmax=vmax)
            else:
            ...
            if vrange is None:
                ...
                cbar.update_normal(t)
            cbar.locator = LinearLocator(numticks=5)  # 保证5个刻度
            cbar.update_ticks()
            ...
            ax.set_title(title, fontsize=title_fontsize, fontweight='bold', family='serif')
            ax.set_aspect('equal')
        ...
        # 设置三个子图
        lo, hi = float(np.min(true_solution)), float(np.max(true_solution))
        exp = np.floor(np.log10(np.maximum(1e-300, np.abs([lo, hi]))))  # 阶次
        fac = 10**(1 - exp)  # sig=2 → 2-1=1
        vrange = (np.floor(lo*fac[0])/fac[0],  np.ceil(hi*fac[1])/fac[1])
        
        _setup_subplot(ax1, true_solution, 'True Solution', vrange=vrange)
        _setup_subplot(ax2, predicted_solution, 'Predicted Solution', vrange=vrange)
        _setup_subplot(ax3, abs_error, 'Absolute Error')  # 误差图不使用固定范围
    ...
    def mesh_with_gauss(points, cells, gauss_points=None, zoom_area=None, save_path="mesh.pdf"):
        ...
        if gauss_points is not None:
            ax.scatter(
                gauss_points[:, 0], gauss_points[:, 1],
                c='red', s=20, linewidths=0, alpha=0.5, zorder=2
            )
            ...
            ax.scatter(
                ...
                s=4, linewidths=0.6, alpha=1.0, zorder=3
            )
...
class MeshTools:
    ...
    @staticmethod
    def convert_torch(points, cells, values, dtype=torch.float64, device='cpu'):
        """
    ...
    @staticmethod
    def calculate_triangle_areas(triangles, coords):
        """
        计算每个三角形的面积
        使用向量叉积公式: area = 0.5 * |(B-A) × (C-A)|
        """
        # 获取每个三角形的三个顶点坐标
        A = coords[triangles[:, 0]]  # (n_tri, 2)
        B = coords[triangles[:, 1]]  # (n_tri, 2)
        C = coords[triangles[:, 2]]  # (n_tri, 2)
        
        # 计算向量
        AB = B - A  # (n_tri, 2)
        AC = C - A  # (n_tri, 2)
        
        # 计算叉积的绝对值 (在2D中，叉积的大小就是行列式的绝对值)
        cross_product = np.abs(AB[:, 0] * AC[:, 1] - AB[:, 1] * AC[:, 0])
        
        # 三角形面积 = 0.5 * 叉积大小
        areas = 0.5 * cross_product
    ...
    @staticmethod
    def generate_oversample_gauss(triangles, coords, values, Lambda, area_threshold=None):
        # 如果指定了面积阈值，过滤三角形
        if area_threshold is not None:
            # 计算每个三角形的面积
            areas = MeshTools.calculate_triangle_areas(triangles, coords)
        ...
        # 获取三角形数据
        triangle_coords = coords[triangles]  # (n_tri, 3, 2)
        triangle_values = values[triangles]  # (n_tri, 3)
        
        # 使用einsum进行批量矩阵乘法
        # 坐标计算: n_tri, n_lam, 2 = n_tri, 3, 2 与 n_lam, 3 的批量点积
        new_X = np.einsum('lk,tkj->tlj', Lambda, triangle_coords)
        new_y = np.einsum('lk,tj->tl', Lambda, triangle_values)
        
        return new_X.reshape(-1, 2), new_y.reshape(-1)
